...
def get_dataset_loader(name, batch_size, num_frames, split='train', hml_mode='train', fixed_len=0, pred_len=0, 
    ...
    # If the dataset has fewer samples than the batch size, don't drop the last
    # (otherwise DataLoader will have zero batches). Use drop_last only when
    # we have at least one full batch to drop from.
    try:
        actual_len = len(dataset)
    except Exception:
        actual_len = None

    drop_last = True if (actual_len is not None and actual_len >= batch_size) else False
