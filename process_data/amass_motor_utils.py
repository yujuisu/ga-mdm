...
# A simple logarithm used for data processing only
# torch_motor_utils has a more compact one
...
def motor_split_log(R: MultiVector, tol=1e-8):
    ...
    tangents = se3_split(tangent)
    norms = norm(tangents[0])
    norms[norms < tol] = 1.0
    if np.any(R.e > 1 + 1e-12) or np.any(R.e < -1 - 1e-12):
        raise ValueError("Input to arccos is outside [-1,1] by more than tolerance.")
    return np.arccos(np.clip(R.e, -1.0, 1.0))*(tangents[0]/norms) + tangents[1]
